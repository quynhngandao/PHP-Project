...
<body class="mb-48">
    ...
    <nav class="flex justify-between items-center mb-4">
        <a href="/" {{-- target="_blank" rel="noopener" --}}><img class="h-24" src="{{ asset('images/phpet-logo.png') }}"
                alt="" class="logo" /></a>
        <ul class="flex space-x-6 mr-6 ml-6 text-lg">
            ...
            {{-- only shown if logged in --}}
                <li>
                    <span class="font-semibold capitalize italic">
                ...
                <li>
                    <form class="inline" method="POST" action="/logout">
                        @csrf
                        <button type="submit">
                            <i class="fa-solid fa-door-closed"></i> <span class="">Logout</span>
                        </button>
                    </form>
                </li>
                @else
    ...
    <footer
        ...
        {{-- Go to Create Route --}}
        <a href="/listings/create" class="absolute top-1/3 right-10 bg-black text-white py-2 px-5">Create Listing</a>
    </footer>
    {{-- FLASH MESSAGE --}}
    <x-flash-message />
</body>
